// ─────────────────────────────────────────────────────────
// media-react/src/useVideos.ts
// Hook for searching & browsing videos with pagination
// ─────────────────────────────────────────────────────────
...
/**
 * Fetch videos — pass a query for search, or omit for popular.
 *
 * ```tsx
 * const { videos, loading, hasMore, loadMore } = useVideos('ocean');
 * const { videos: popular } = useVideos(); // popular/trending
 * ```
 */
    ...
    async (pageNum: number, append: boolean) => {
      ...
      try {
        ...
        if (query && query.trim().length > 0) {
          ...
          result = await client.getPopularVideos({
            page: pageNum,
            per_page: perPage,
          });
        }
